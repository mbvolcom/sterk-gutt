...
function openSessionEditor(sessionId) {
  ...
  if (stravaConnected) {
    ...
    stravaBtn.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="#FC4C02"><path d="M15.387 17.944l-2.089-4.116h-3.065L15.387 24l5.15-10.172h-3.066m-7.008-5.599l2.836 5.598h4.172L10.463 0l-7 13.828h4.169"/></svg>${alreadyUploaded ? '↺ Re-upload to Strava' : 'Upload to Strava'}`;
    if (alreadyUploaded) {
      stravaBtn.style.opacity = '0.65';
    }
...
const PM_TO_BODY_IDS = {
  ...
  'Calves':                ['tibialis-anterior-left','tibialis-anterior-right','calves-gastroc-medial-left','calves-gastroc-lateral-left','calves-soleus-left','calves-gastroc-medial-right','calves-gastroc-lateral-right','calves-soleus-right'],
...
const BODY_ID_TO_LABEL = {
  ...
  'neck-left':'Traps','neck-right':'Traps','nape':'Traps',
...
function renderMuscleMap(containerId, muscleData, heatMode = false, secondaryData = {}) {
  ...
  return _renderBM(wrap, tooltip, muscleData, secondaryData, heatMode, lib);
}
...
function openFullscreenMap(muscleData, secondaryData, heatMode) {
  ...
  setTimeout(() => {
    ...
    fsFront = new lib.BodyChart(frontEl, { view: lib.ViewSide.FRONT, bodyState, enableTransitions: false, onMuscleClick });
    fsBack  = new lib.BodyChart(backEl,  { view: lib.ViewSide.BACK,  bodyState, enableTransitions: false, onMuscleClick });
  }, 50);
}
...
async function uploadSessionToStrava(session, btnEl) {
  ...
  try {
    ...
    // Mark session as uploaded in memory and Supabase
    session.stravaUploaded = true;
    const allSessions = load(SK.sessions) || [];
    ...
    dbSaveSession(session).catch(e => console.warn('Strava flag save failed:', e.message));
    btnEl.innerHTML = `<svg width="16" height="16" viewBox="0 0 24 24" fill="#FC4C02"><path d="M15.387 17.944l-2.089-4.116h-3.065L15.387 24l5.15-10.172h-3.066m-7.008-5.599l2.836 5.598h4.172L10.463 0l-7 13.828h4.169"/></svg>↺ Re-upload to Strava`;
